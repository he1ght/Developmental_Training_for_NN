...
ft = open('cifar{}_train_result.csv'.format(args.cifar), 'w', newline='')
wrt = csv.writer(ft)
wrt.writerow(['', 'seeds', 'train_nll', 'train_te1', 'train_te3', 'train_te5', 'train_te1_std', 'train_te3_std'])
f = open('cifar{}_test_result.csv'.format(args.cifar), 'w', newline='')
wr = csv.writer(f)
wr.writerow(['', 'seeds', 'test_nll', 'test_te1', 'test_te3', 'test_te5', 'test_te1_std', 'test_te3_std'])
...
prefix = 'CIFAR100_' if args.cifar == 100 else 'CIFAR10_'

for checkpoint_name in name_list:
    if not checkpoint_name.split('/')[-1].startswith(prefix):
        # print(checkpoint_name)
        continue
    train_result = []
    result = []
    __seed = 1
    for _seed in range(1, args.seeds + 1):
        try:

            checkpoint_seed_name = checkpoint_name + str(_seed) + '.pt'
            ...
            seed_train_result = train(model)
            seed_result = test(model)
            train_result.append(seed_train_result)
            result.append(seed_result)
            __seed = _seed
    ...
    if __seed > 0:
        ...
        wrt.writerow([checkpoint_name, __seed, *train_result.mean(axis=0), *train_result.std(axis=0)[1:3]])
        wr.writerow([checkpoint_name, __seed, *result.mean(axis=0), *result.std(axis=0)[1:3]])
        print(checkpoint_name)
