...
def train(epoch, model, loss_fn):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        loss = loss_fn(output, target, teacher_output_1, teacher_output_2, T=args.T, alpha=alpha_t, beta=beta_t)
        loss.backward()
        optimizer.step()
...
def test(epoch, model, loss_fn):
    ...
    with torch.no_grad():
        ...
        for data, target in test_loader:
            ...
            teacher_output_2 = teacher_model_2(data)
            teacher_output_2 = teacher_output_2.detach()
            test_loss += loss_fn(output, target, teacher_output_1, teacher_output_2, T=args.T,
...
for epoch in range(1, args.epochs + 1):
    ...
    #     adjust_lr(rate=0.2)
    train(epoch, model, loss_fn=double_distillation)
    test(epoch, model, loss_fn=double_distillation)
