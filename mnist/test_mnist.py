...
num_classes = 10
result = [0., 0., 0., 0.]
for seed_int in range(1, args.seeds + 1):
    checkpoint = torch.load(args.checkpoint + str(seed_int) + '.pt', map_location=lambda storage, loc: storage.cuda(args.gpu))
    ...
    for i in range(len(result)):
        result[i] += seed_result[i]
mean_result = [res/args.seeds for res in result]
print("Test seeds {}| NLL: {:.4f}, T1: {:.4f}, T3: {:.4f}, T5: {:.4f}".format(args.seeds, *mean_result))
