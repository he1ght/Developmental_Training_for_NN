...
def smoothed_distillation(y, labels, teacher_scores, alpha, epsilon):
    return softmax_cross_entropy_with_softtarget(y, F.softmax(teacher_scores, dim=1)) * alpha \
           + softmax_cross_entropy_with_softtarget(y, torch.ones_like(y) / y.size(-1)) * epsilon \
           + F.cross_entropy(y, labels) * (1. - alpha - epsilon)
...
def train(epoch, model, loss_fn):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if args.ada_alpha:
            ...
            alpha_t = args.alpha
        loss = loss_fn(output, target, teacher_output, alpha=alpha_t, epsilon=args.ls)
        if args.l1 > 0:
            l1_loss = 0
...
def test(epoch, model, loss_fn):
    ...
    for data, target in test_loader:
        ...
        test_loss += loss_fn(output, target, teacher_output,
                             alpha=alpha_t, epsilon=args.ls).detach()
...
for epoch in range(1, args.epochs + 1):
    train(epoch, model, loss_fn=smoothed_distillation)
    test(epoch, model, loss_fn=smoothed_distillation)
