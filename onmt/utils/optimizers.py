...
def build_torch_optimizer(model, opt):
    ...
    if opt.optim == 'sgd':
        optimizer = optim.SGD(params, lr=opt.learning_rate, weight_decay=opt.weight_decay)
    elif opt.optim == 'adagrad':
        optimizer = optim.Adagrad(
            params,
            lr=opt.learning_rate,
            initial_accumulator_value=opt.adagrad_accumulator_init, weight_decay=opt.weight_decay)
    elif opt.optim == 'adadelta':
        optimizer = optim.Adadelta(params, lr=opt.learning_rate, weight_decay=opt.weight_decay)
    ...
    def from_opt(cls, model, opt, checkpoint=None):
        ...
        if opt.train_from and checkpoint is not None:
            ...
            if opt.reset_optim == 'none':
                ...
                optim_state_dict = ckpt_state_dict
            elif opt.reset_optim == 'keep_steps':
                # For Transfer Learning
                ckpt_state_dict.pop('optimizer')
                optim_state_dict = ckpt_state_dict
