...
class skd_criterion(nn.Module):
    ...
    def nll_with_softtarget(self, input, target, reduction='sum'):
            """
            :param input: (batch, *)
            :param target: (batch, *) same shape as input, each item must be a valid distribution: target[i, :].sum() == 1.
            """
            logprobs = input.view(input.shape[0], -1)
            batchloss = - torch.sum(target.view(target.shape[0], -1) * logprobs, dim=1)
            if reduction == 'none':
                return batchloss
            elif reduction == 'mean':
...
def build_loss_compute(model, tgt_field, opt, train=True, do_backward=True, teacher=None):
    """
    ...
    if opt.copy_attn:
        ...
        do_backward = do_backward, teacher=teacher)
    compute.to(device)
...
class LossComputeBase(nn.Module):
    ...
    def _make_shard_state(self, batch, output, range_, attns=None, t_out=None):
        """
    ...
    def __call__(self,
        ...
        shard_state = self._make_shard_state(batch, output, trunc_range, attns, t_out)
        t_output = t_out[0] if t_out is not None else None
        if shard_size == 0:
            loss, stats = self._compute_loss(batch, **shard_state) #, t_output=t_output)
        ...
        for shard in shards(shard_state, shard_size, do_backward=self.do_backward):
            loss, stats = self._compute_loss(batch, **shard) #, t_output=t_output)
...
class NMTLossCompute(LossComputeBase):
    ...
    def __init__(self, criterion, generator, normalization="sents",
                 lambda_coverage=0.0, lambda_align=0.0, do_backward=True, teacher=None):
    ...
    def _compute_loss(self, batch, output, target, std_attn=None,
                      coverage_attn=None, align_head=None, ref_align=None, t_output=None):
        ...
        if t_output is not None:
            with torch.no_grad():
                # if target.size(0) < t_output.size(0):
                #     t_output = t_output[:target.size(0),:,:]
                t_score = self.teacher.generator(self._bottle(t_output))
                # gtruth_onehot = torch.nn.functional.one_hot(gtruth, t_score.size(-1))
            loss = self.criterion(scores, gtruth, t_score)
        else:
